...
void Graph::addNode(char *str){
	...
	x = atof(strtok(NULL, ","));
	y = atof(strtok(NULL, ","));
	if(id%2){ // id impar e maior que dois (tipo... todos os impares que não seja 1)
			if(id-2 > 0){
				adjList[id].push_back(id-2);
				adjList[id-2].push_back(id);
			}
			if(id+1 < 220){
				adjList[id].push_back(id+1);
				adjList[id+1].push_back(id);
			}
	} else if(!(id%2)){ // Id par e maior que 1 (tipo... todos os pares)
		if(id+2 < 220){
			adjList[id].push_back(id+2);
			adjList[id+2].push_back(id);
		}
